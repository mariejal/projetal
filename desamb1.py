...
plt.tight_layout()
plt.legend()
plt.savefig("8comparaison_supervise_%s" % d.vb_choisi)
"""
"""
...
plt.title("Evolution des performance Kmeans pour un lancer\nNon supervisé\n%s" % d.vb_choisi)
plt.tight_layout()
#plt.legend()
...
for i in range(100):
	...
	if d.version == "non_superv":
		non_superv()
		print("résultat de ce %s ieme lancer - non supervisé: " % str(i+1), d.kmeans.resultats)	
		res_lances += [max(d.kmeans.resultats)]

	# 2) supervisé faible
	if d.version == "faible_superv":
		faible_superv()
		print("résultat de ce %s ieme lancer - supervisé faible: " % str(i+1), d.kmeans.resultats)
		res_lances += [max(d.kmeans.resultats)]

	# 3) supervisé fort
	if d.version == "forte_superv":
		forte_superv()
		print("résultat de ce %s ieme lancer - supervisé fort: " % str(i+1), d.kmeans.resultats)
		res_lances += [max(d.kmeans.resultats)]
...
plt.ylabel("taux de bonnes réponses")
plt.xlabel("i-eme lancé du Kmeans")
plt.title("Performance Kmeans sur %s lancers\n%s" % (str(len(y)), d.vb_choisi))
fig.canvas.draw()
plt.tight_layout()
...
for i in range(len(liste_pourcentages)):
	
	# on écrase l'ancien pourcentage à chaque fois
	d.pourcentage_graine = liste_pourcentages[i]
...
y = [round(res) for res in res_pourcentages]
...
plt.title("Performance Kmeans selon pourcentage d'ex utilisés pr graines\n%s\n%s" % (d.version, d.vb_choisi))
fig.canvas.draw()
plt.tight_layout()
plt.savefig("résultats_selon_pourcent_graines%s_%s" % (d.vb_choisi, d.version))
