...
class Desamb:
	...
	def createX_Y(self, use_seeds):
		...
		# a revoir si on veut tronquer les données
		"""
...
class K_Means:
	...
	def __init__(self, k, g, tol=0.001, max_iter=10, seeds=[],):
		self.k = k
		self.tol = tol
...
#tu choisis 1 version parmi les 3 possibles
...
for i in range(lances_Kmeans):
	...
	# 1) non supervisé
	d.createX_Y(False) # pas de seeds
	d.kmeans.fit(d.X, d.Y, False, False) # pas de seeds + pas de maj controlees
	print("résultat de ce %s ieme lancé - non supervisé: " % str(i+1), d.kmeans.resultats)	
	res_nSup += [max(d.kmeans.resultats)]

	# 2) supervisé faible
	d.createX_Y(True) # seeds Ok
	d.kmeans = K_Means(k=len(d.gold2vec), seeds=d.seeds, g=d.g) #on réinstancie le kmeans avec nos seeds
	d.kmeans.fit(d.X, d.Y, True, False) # seeds Ok + pas de maj controlees
	...
	d.createX_Y(True) # seeds Ok
	d.kmeans = K_Means(k=len(d.gold2vec), seeds=d.seeds, g=d.g) #on réinstancie le kmeans avec nos seeds
	d.kmeans.fit(d.X, d.Y, True, True) # seeds Ok + maj controlees Ok
...
d.createX_Y(False) # pas de seeds
d.kmeans.fit(d.X, d.Y, False, False) # pas de seeds + pas de maj controlees
print("évoltion des résultats - non supervisé: ", d.kmeans.resultats)	
...
for i in range(lances_Kmeans):
	...
	# tu choisis ta version comme dans la partie normale
